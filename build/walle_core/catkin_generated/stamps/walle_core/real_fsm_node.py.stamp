...
class RealSortingFSM:
    ...
    def grab_once(self, color):
        ...
        # 2. 先收低并闭合夹爪，再用底盘完成相机闭环对位。
        # 这样底盘前进时不会用张开的夹爪扫桌面。
        self.set_state('stow')
        rospy.loginfo('收起并闭合夹爪，准备空手底盘对位')
        self.arm_cmd(0.0, self.gripper_close)
        rospy.sleep(1.0)
        ...
        # 3. 对位完成后强制停车，再抬臂并张开夹爪；此后不再让底盘前进。
        self.stop_base()
        rospy.sleep(0.5)
        self.set_state('hand_up')
        rospy.loginfo('底盘已停止，抬臂到 %.2fm 并张开夹爪', self.lift_grab_height)
        self.arm_cmd(self.lift_grab_height, self.gripper_open)
        rospy.sleep(2.0)
        self.stop_base()

        # 4. 立刻闭合夹爪。即使相机被手臂短暂遮挡，也不要再移动底盘。
        self.set_state('grab')
        rospy.loginfo('闭合夹爪')
