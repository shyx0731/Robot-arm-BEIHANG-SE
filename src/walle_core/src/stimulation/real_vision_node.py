#!/usr/bin/env python3
# real_vision_node.py
# 订阅 Gazebo Kinect 图像，检测彩色箱子，通过 TF 发布 map 坐标
    ...
    def rgb_cb(self, msg):
        ...
        for color_name, ranges in color_ranges.items():
            ...
            for contour in contours:
                ...
                # This is the only data the grasp alignment should use; it is
                # independent from map/AMCL and from other boxes' depths.
                cam_x = depth_meters * (sd_cx - self.sd_cx) / self.sd_fx
                cam_y = depth_meters * (sd_cy - self.sd_cy) / self.sd_fy

                camera_msg = Pose()
                camera_msg.position.x = cam_x
                camera_msg.position.y = cam_y
                ...
                camera_msg.orientation.z = float(cv2.contourArea(contour))
                ...
                # 不做持久记录，每帧都发布当前看到的箱子，供 FSM 实时对位
                if world_pos is not None:
                    ...
                    try:
                        ...
                        return  # 节点正在关闭，话题已关，直接退出回调

                rospy.loginfo_throttle(
                    2.0,
                    '看到 %s 箱子: camera=(x %.3f, depth %.2f)',
                    color_name, cam_x, depth_meters)

                cv2.rectangle(cv_image, (x, y), (x + w, y + h), (0, 255, 0), 2)
                label = '%s %.2fm' % (color_name, depth_meters)
